...
| `SUBTITLE_DIR` | `/data/subtitles` | Директория для файлов субтитров |
...
| `/videos/{id}` | Страница видео — плеер, метаданные, управление тегами и субтитрами |
...
├── subtitles/     # Субтитры (.srt / .vtt) по видео
└── app.db         # SQLite (если не используется PostgreSQL)
```

## Субтитры

На странице видео можно загрузить субтитры в формате .srt или .vtt (SRT конвертируется в WebVTT автоматически, поддерживаются UTF-8/UTF-16/Windows-1251). В плеере появляется кнопка CC, как на YouTube:

- меню дорожек с выбором языка/метки, переключение по клавише `C`;
- выбор запоминается в браузере (localStorage);
- дорожка, помеченная «по умолчанию», включается автоматически;
- субтитры работают и при перекодировании в HLS, и при просмотре исходника.
